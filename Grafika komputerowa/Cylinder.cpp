    ...
    int stacks, bool smooth, int up) : interleavedStride(32)
{
    ...
    translation[0] = translation[1] = translation[2] = 0.0f; // initialize translation to zero
}

void Cylinder::setTranslation(float x, float y, float z)
{
    translation[0] = x;
    translation[1] = y;
    translation[2] = z;
...
void Cylinder::buildVerticesSmooth()
{
    ...
    for (int i = 0; i <= stackCount; ++i)
    {
        ...
        for (int j = 0, k = 0; j <= sectorCount; ++j, k += 3)
        {
            ...
            addVertex(x * radius + translation[0], y * radius + translation[1], z + translation[2]);   // position
    ...
    z = -height * 0.5f;
    addVertex(translation[0], translation[1], z + translation[2]);
    addNormal(0, 0, -1);
    ...
    for (int i = 0, j = 0; i < sectorCount; ++i, j += 3)
    {
        ...
        addVertex(x * baseRadius + translation[0], y * baseRadius + translation[1], z + translation[2]);
        addNormal(0, 0, -1);
    ...
    for (int i = 0, j = 0; i < sectorCount; ++i, j += 3)
    {
        x = unitCircleVertices[j];
        y = unitCircleVertices[j + 1];
        addVertex(x * topRadius + translation[0], y * topRadius + translation[1], z + translation[2]);
        addNormal(0, 0, 1);
...
void Cylinder::buildVerticesFlat()
{
    ...
    for (i = 0; i <= stackCount; ++i)
    {
        ...
        for (j = 0, k = 0; j <= sectorCount; ++j, k += 3)
        {
            ...
            Vertex vertex;
            vertex.x = x * radius + translation[0];
            vertex.y = y * radius + translation[1];
            vertex.z = z + translation[2];
            vertex.s = s;
            vertex.t = t;
    ...
    for (i = 0; i < stackCount; ++i)
    {
        ...
        for (j = 0; j < sectorCount; ++j, ++vi1, ++vi2)
        {
            ...
            addIndices(index + 2, index + 3, index);
    ...
    addNormal(0, 0, -1);
    addTexCoord(0.5f, 0.5f);
    for (i = 0, k = 0; i < sectorCount; ++i, k += 3)
    {
        x = unitCircleVertices[k];
        y = unitCircleVertices[k + 1];
        addVertex(x * baseRadius + translation[0], y * baseRadius + translation[1], z + translation[2]);
    ...
    // put indices for base
    for (i = 0, k = (unsigned int)vertices.size() / 3 - sectorCount; i < sectorCount; ++i, ++k)
    {
        if (i < (sectorCount - 1))
            addIndices(baseIndex, k + 1, k + 2);
        else
            addIndices(baseIndex, k + 1, baseIndex + 1);
    }
    ...
    {
        if (i < (sectorCount - 1))
            addIndices(topIndex, k + 2, k + 1);
        else
            addIndices(topIndex, topIndex + 1, k + 1);
    }
